...
def scan_files(
    ...
    current_issue_files = {
        f["filepath"]: f["id"] for f in volume.get_all_files()
    }
    ...
    for file in filtered_iter(folder_contents, set(filepath_filter)):
        if file in manually_matched_files:
            ...
            manually_matched_files_found.add(manually_matched_files.pop(file))
            continue
        ...
        if (
            ...
            if matching_issues:
                ...
                for issue in matching_issues:
                    new_issue_bindings.add((current_issue_files[file], issue))
    ...
    current_bindings: set[tuple[int, int]] = set(
        map(
            tuple,
            cursor.execute(
                """
    ...
    deleted_downloaded_issues = [
        k for k, v in issue_binding_count.items() if not v
    ]
    ...
    for file_id, _issue_id in new_issue_bindings:
...
def get_file_matching(volume_id: int) -> list[FileMatch]:
    ...
    volume_folder: str = cursor.execute(
        "SELECT folder FROM volumes WHERE id = ? LIMIT 1;", (volume_id,)
    ).fetchone()[0]
    ...
    for file_match in matches:
        ...
        if not file_match["forced_match"]:
            ...
            if not file_match["general_file"]:
                ...
                cursor.execute(
                    "DELETE FROM volume_files WHERE file_id = ?;", (file_id,)
                )
