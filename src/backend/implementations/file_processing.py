...
def mass_set_file_date(
    ...
    cursor.execute(
        ...
        """,
        {"volume_id": volume_id, "issue_id": issue_id or -1},
    )
...
def mass_set_permissions(volume_id: int) -> None:
    ...
    volume_folder, root_folder_id = (
        get_db()
        .execute(
            "SELECT folder, root_folder FROM volumes WHERE id = ?", (volume_id,)
        )
        .fetchone()
    )

    set_volume_folder_permissions(
        volume_folder, RootFolders()[root_folder_id], permissions
    )
...
def mass_set_ownership(volume_id: int) -> None:
    ...
    set_volume_folder_owner_group(
        volume_folder, RootFolders()[root_folder_id], group
    )
