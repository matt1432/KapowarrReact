...
class RemoteMapping:
    ...
    def update(
        self,
        external_download_client_id: int | None = None,
    ...
    def delete(self) -> None:
        ...
        get_db().execute(
            "DELETE FROM remote_mappings WHERE id = ?;", (self.id,)
        )
        return
...
class RemoteMappings:
    ...
    @classmethod
    def local_to_remote(
        cls, external_download_client_id: int, path: str
    ) -> str:
    ...
    @classmethod
    def add(
        cls, external_download_client_id: int, remote_path: str, local_path: str
    ) -> RemoteMapping:
