...
class KapowarrCursor(Cursor):
    ...
    def connection(self) -> DBConnection:
        return super().connection  # pyright: ignore

    def __init__(self, cursor: DBConnection, /) -> None:
        super().__init__(cursor)
        return

    def fetchonedict(self) -> dict[str, Any] | None:
    ...
    def fetchmanydict(self, size: int | None = 1) -> list[dict[str, Any]]:
...
class DBConnectionManager(type):
    ...
    def close_connection_of_thread(cls) -> None:
        ...
        thread_id = current_thread_id()
        if thread_id in cls.instances and not cls.instances[thread_id].closed:
...
def iter_commit[T](iterable: Iterable[T]) -> Iterator[T]:
