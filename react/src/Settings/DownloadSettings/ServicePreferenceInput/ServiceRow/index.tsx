...
interface ServiceRowProps {
    name: GCDownloadSource;
    index: number;
    isDraggingDown: boolean;
    isDraggingUp: boolean;
    onServiceRowDragEnd: (didDrop: boolean) => void;
    onServiceRowDragMove: (dragIndex: number, hoverIndex: number) => void;
}
...
export default function ServiceRow({
    ...
    const [{ isOver }, dropRef] = useDrop<DragItem, void, { isOver: boolean }>({
        accept: DragType.ServicePreference,
        collect(monitor) {
            return {
        ...
        hover(item: DragItem, monitor) {
            ...
            onServiceRowDragMove(dragIndex, hoverIndex);
        },
    });
    ...
    const [{ isDragging }, dragRefConnector, previewRef] = useDrag<
        ...
        { isDragging: boolean }
    >({
        type: DragType.ServicePreference,
        item: () => {
            return {
                name,
                index,
        ...
        collect: (monitor: DragSourceMonitor<unknown, unknown>) => ({
            ...
            onServiceRowDragEnd(monitor.didDrop());
        },
    });
