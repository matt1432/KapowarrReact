...
interface ComicsTag extends TagBase {
    id: number;
    name: string;
}

export interface ComicsTagInputProps<V> {
    name: string;
    value: V;
...
function createComicsTagsSelector(tags: number[]) {
    return createSelector(createTagsSelector(), (tagList) => {
        ...
        return {
            tags: tags.reduce((acc: ComicsTag[], tag) => {
...
export default function ComicsTagInput<V extends number | number[]>({
    name,
    value,
    onChange,
    ...
    const { tags, tagList, allTags } = useSelector(createComicsTagsSelector(arrayValue));
