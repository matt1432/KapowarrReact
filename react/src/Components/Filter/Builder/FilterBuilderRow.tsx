...
import { type FilterBuilderTypes, possibleFilterTypes } from 'Helpers/Props/filterBuilderTypes';
...
function getselectedFilterBuilderProp<T>(filterBuilderProps: FilterBuilderProp<T>[], name: string) {
...
function getDefaultFilterValue<T>(selectedFilterBuilderProp: FilterBuilderProp<T>) {
...
function getRowValueConnector<T>(selectedFilterBuilderProp: FilterBuilderProp<T> | undefined) {
    if (!selectedFilterBuilderProp) {
...
function FilterBuilderRow<T>({
    ...
    const handleFilterValueChange = useCallback(
        ({ value }: InputChanged<string | boolean[] | string[] | number[] | DateFilterValue>) => {
            if (filterKey == null || filterValue == null || filterType == null) {
                return;
            }
    ...
    useEffect(() => {
        if (filterKey) {
            setSelectedFilterBuilderProp(filterBuilderProps.find((a) => a.name === filterKey));
