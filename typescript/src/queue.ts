...
function addQueueEntry(api_key, obj) {
    ...
    entry.querySelector('.move-up-dl').onclick = () => moveEntry(
        obj.id, index - 1, api_key,
    );
    entry.querySelector('.move-down-dl').onclick = () => moveEntry(
        obj.id, index + 1, api_key,
    );
    entry.querySelector('.remove-dl').onclick = () => deleteEntry(
        obj.id, api_key,
    );
    entry.querySelector('.blocklist-dl').onclick = () => {
        deleteEntry(
            obj.id,
            api_key,
            blocklist = true,
...
function updateQueueEntry(obj) {
    ...
    tr.querySelector('td:nth-child(1)').innerText = obj.status.charAt(0)
        .toUpperCase() + obj.status.slice(1);
    tr.querySelector('td:nth-child(4)').innerText = convertSize(obj.size);
    tr.querySelector('td:nth-child(5)').innerText = `${twoDigits(
        Math.round(obj.speed / 100000) / 10,
    )}MB/s`;
    tr.querySelector('td:nth-child(6)').innerText = obj.size === -1 ?
